...
| 16 | 이력서 조회/저장 | PARTIAL | 임시 학생으로 Django API 생성·수정→RDS 행·bootstrap 재조회 성공. React가 생성 시 임시 ID 대신 서버 ID를 기다리도록 수정한 뒤 실제 브라우저 생성 200, `/resume/3/edit`에서 조회 성공. 테스트 계정·이력서 삭제 후 잔여 0건. 편집 화면의 저장 버튼과 AI 첨삭은 별도 검증 필요. |
