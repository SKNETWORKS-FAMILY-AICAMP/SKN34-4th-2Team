...
function mapSeating(payload: Record<string, unknown>, fallbackCohort: string): SeatingLayout {
  const seating = payload.seating as Record<string, unknown> | undefined;
  const room = (seating?.room ?? {}) as Record<string, unknown>;
  const cells = Array.isArray(seating?.cells) ? (seating?.cells as Record<string, unknown>[]) : [];
  const seats = Array.isArray(seating?.seats) ? (seating?.seats as Record<string, unknown>[]) : [];
  if (!seating || cells.length === 0) {
    return emptyDb().seating;
  }
  return {
    id: String(room.id ?? room.pk ?? payload.publishedSeatingRoomId ?? ''),
    cohortId: String(room.cohortId ?? fallbackCohort),
    rows: Number(room.rows ?? room.rowCount ?? 0),
    cols: Number(room.cols ?? room.colCount ?? 0),
    roomNumber: room.roomNumber || room.name ? String(room.roomNumber ?? room.name) : undefined,
    published: true,
    cells: cells.map((cell) => ({
      seatId: String(cell.seatId ?? cell.id ?? cell.pk ?? ''),
      row: Number(cell.row ?? 0),
      col: Number(cell.col ?? 0),
      label: String(cell.label ?? ''),
      type: (cell.type as SeatingLayout['cells'][number]['type']) || 'seat',
      groupId: cell.groupId ? String(cell.groupId) : undefined,
    })),
    seats: seats.map((seat) => ({
      seatNumber: Number(seat.seatNumber ?? seat.seat_number ?? 0),
      row: Number(seat.row ?? 0),
      col: Number(seat.col ?? 0),
      userId: seat.userId ? String(seat.userId) : undefined,
      userDisplayName: seat.userDisplayName ? String(seat.userDisplayName) : undefined,
    })),
    updatedAt: asDate(room.updatedAt ?? room.updated_at),
  };
}
...
export function mapBootstrap(payload: Record<string, unknown>): Database {
  ...
  return {
    ...
    seating: mapSeating(payload, sessionCohort),
